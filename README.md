...
### 배포 환경에서 allauth 설정하기
> 배포 환경에서, 기존에 알고 있던 allauth의 설정에는 local 쪽의 설정이 많았기에 제대로 라이브러리가 작동하지 않는 문제가 있었다. 

1. site 설정 변경 : admin에 들어가 사이트 > 사이트들 에 들어가 도메인을 본다면 로컬 기준이었을 때에는 localhost:8000이 되어 있었을 것이다. 이것을 자신의 배포 사이트 url로 변경하여 준다. **http://**를 포함시켜 주어야 한다.

2. 이후, 소셜계정 > 소셜 어플리케이션으로 들어가, 계정을 생성한다.
클라이언트 아이디와 비밀번호를 입력, 아래의 Sites에는 1번에서 변경하였던 도메인을 오른쪽으로 보낸다. 
3. 이후 소셜 로그인 시도 시, 발생하는 콜백 에러의 url을 google기준에서 설명한다면 승인된 리디렉션 URL에 시도 시 떳던 콜백에러의 url을 넣으려 하면 승인된 url을 넣으라고 한다. 

4. OAuth 동의화면으로 넘어가 승인된 도메인에 1번에서 추가하였던 도메인을 넣어주는데 http://를 생략하고 추가하였더니 소셜 로그인이 완료되었다.

5. 네이버의 기준에서는 서비스 url에 배포한 도메인을 넣고

6. 리디렉션에 배포한 url/accounts/naver/login/callback/을 넣어주면 allauth가 정상 작동한다.
   
### 설정 - 네이버
#### settings
AUTHENTICATION_BACKENDS = [
...
```ml-md-auto : 왼쪽 오른쪽 여백을 똑같이 한다. ```

**div의 크기가 992px 이상일 떄**
```div.offset-lg-<int> : 요소의 시작을 열 기준 <int>/12에서시작한다.``` 

```div.col-lg-<int> : 요소의 크기는 열 기준 <int>/12의 크기를 가진다 ``` 
**여기까지**

```<div class="conainter-fluid"></div>: container-fluid 클래스를 부여하면 화면의 크기에 따라 다르게 반응할 수 있는 wrapper를 사용하게 되는 것이다```

### 그리드 시스템
[부트 스트랩 참고 사이트](https://unikys.tistory.com/371)
>Bootstrap이 가장 많이 사용되는 이유이자 반응형 웹페이지를 만들기 위해서 가장 많이 사용이 필요한 기능이다.


>Bootstrap에서는 기본적으로 12칼럼으로 나누어서 기능을 제공하고 있다고 생각하면 쉬울 것이다. 이를 테이블의 형식대로 row 클래스와 각종 col-* 클래스들로 자유롭게 구성이 가능하며, 이렇게 구성한 페이지들은 Bootstrap에서 알아서 크기에 따라 반응형으로 동작하도록 만드는 것이다. Bootstrap에서는 아래의 규칙대로 이러한 그리드 시스템을 구축하라고 권고하고 있다.
